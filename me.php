...
<?php if(HotelStatus() == "online"){ ?>
		<div class="open enter-btn">
				<a href="<?php echo PATH; ?>/beta" target="client" ><?php echo $lang->loc['enter.short']; ?><i></i></a>
			<b></b>
		</div>
<?php } else { ?>
...
<?php } ?>
	...
	<ul id="link-bar" class="clearfix">
		<li class="credits">
			<a href=""><?php echo $user->user("credits"); ?></a> <?php echo $lang->loc['credits']; ?>
		</li>
...
?>
        ...
        <li class="<?php echo $even; ?>" style="background-image: url(<?php echo BADGE; ?>/habbo-imaging/badge/<?php echo $row[2]; ?>.gif)">
...
while($row = $db->fetch_row($sql)){
	...
	echo "            <li class=\"".$oddeven." ".$pos."\" style=\"background-image: url(".BADGE."/habbo-imaging/badge/".$row[2].".gif)\">\n            	\n                \n                <a class=\"item\" href=\"".groupURL($row[0])."\">".$input->HoloText($row[1])."</a>\n            </li>";
	$i++;
}
