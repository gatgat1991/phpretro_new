...
while ($row = mysqli_fetch_object($sql)) {
	...
	if($row->users == 0){ $row->users = 1; }
	$count[$i] = ($row->users / $row->users_max) * 100;
...
?>
<li class="<?= $even; ?>">
	<span class="clearfix enter-room-link room-occupancy-<?= $room_fill; ?>" title="<?= $lang->loc['go.to.room']; ?>" roomid="<?= $row->id; ?>">
		<span class="room-enter"><?php echo $lang->loc['enter']; ?></span>
		<span class="room-name"><?= $row->name; ?></span>
		<span class="room-description"><?= $row->description; ?></span>
		<span class="room-owner"><?= $lang->loc['owner']; ?>: <a href="<?php echo PATH; ?>/home/<?php echo $row->owner_name; ?>" "><?= $row->owner_name; ?></a></span>
	</span>
<?php } ?>
