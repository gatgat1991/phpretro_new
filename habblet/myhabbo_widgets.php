...
<?php } ?>
	...
	<div class="profile-info">
		...
		<div>
    	<?php $bsql = $data->select10($userrow[0]); if($db->num_rows($bsql) != 0){ $brow = $db->fetch_row($bsql); ?><a href="<?php echo groupURL($brow[0]); ?>" title="<?php echo $input->HoloText($brow[2]); ?>"><img src="<?php echo BADGE; ?>/habbo-imaging/badge/<?php echo $input->HoloText($brow[1]); ?>.gif" /></a><?php } ?>
...
<?php while($grow = $db->fetch_row($gsql)){ ?>
	...
	<li title="<?php echo $input->HoloText($grow[2]); ?>" id="groups-list-<?php echo $widgetrow[0]; ?>-<?php echo $grow[0]; ?>">
		<div class="groups-list-icon"><a href="<?php echo groupURL($grow[0]); ?>"><img src="<?php echo BADGE; ?>/habbo-imaging/badge/<?php echo $grow[1]; ?>.gif"/></a></div>
