		...
		switch($mode){
			...
			case 2:
				...
				while($row = $db->fetch_row($sql)){
					...
					if($row[4] >= "2"){ echo "<a href=\"".PATH."/client?forwardId=2&amp;roomId=".$row[5]."\" onclick=\"HabboClient.roomForward(this, '".$row[5]."', 'private'); return false;\" target=\"client\" class=\"group-room\" title=\"".$lang->loc['group.room']."\"></a>"; }
					if($row[5] == 1){ echo "<div class=\"favourite-group\" title=\"".$lang->loc['favorite']."\"></div>\n"; }
					if($row[2] == 1 && $row[3] != $user->id){ echo "<div class=\"admin-group\" title=\"".$lang->loc['administrator']."\"></div>\n"; }
					if($row[2] == 0 && $row[3] == $user->id){ echo "<div class=\"owned-group\" title=\"".$lang->loc['owner']."\"></div>\n"; }
