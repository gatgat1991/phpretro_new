...
class HoloInput {
	...
	function FilterText($str) {
		...
		if($GLOBALS['conn']['main']['server'] == "mysql" || $GLOBALS['conn']['server']['server'] == "mysql"){
		//var_dump($GLOBALS); 
		$mysqli = new mysqli($GLOBALS['conn']['main']['host'], $GLOBALS['conn']['main']['username'], $GLOBALS['conn']['main']['password'], $GLOBALS['conn']['main']['database']);
	...
	function HoloText($str, $advanced=true) {
...
class HoloUser {
	...
	function avatarURL($figure,$style,$return = 0){
		...
		if($GLOBALS['settings']->find("site_cache_images") == "1" && file_exists("./cache/avatars/".$figure.",".$expandedstyle.",".$hash.".png")){
			...
			$look = 'hr-545-32.hd-625-2.ch-645-66.lg-695-89.sh-730-88.he-1602-73';
			$URL = "https://www.habbo.de/habbo-imaging/avatarimage?figure=".$figure."&size=".$style[0]."&direction=".$style[1]."&head_direction=".$style[2]."&crr=".$style[5]."&gesture=".$style[3]."&frame=".$style[4];
			$curl_handle=curl_init();
			//This is the URL you would like the content grabbed from
			curl_setopt($curl_handle, CURLOPT_USERAGENT, 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10.9; rv:32.0) Gecko/20100101 Firefox/32.0');
			curl_setopt($curl_handle, CURLOPT_SSL_VERIFYPEER, false);
			curl_setopt($curl_handle, CURLOPT_REFERER, 'https://www.habbo.de/');
			...
			$URL = "https://habbo.de/habbo-imaging/avatarimage?figure=".$figure."&size=".$style[0]."&direction=".$style[1]."&head_direction=".$style[2]."&crr=".$style[5]."&gesture=".$style[3]."&frame=".$style[4];
		}
	...
	function HoloDatabase($conn){
		switch($conn['server']){
			case "mysql":
				...
				$this->connection or $this->error = mysqli_connect_error();
				break;
			 case "pdomysql":
				$this->connection = new PDO("mysql:host=localhost;dbname=playfirs1_habb0", $conn['username'], $conn['password']);
				break;
				/*
			case "sqlite":
