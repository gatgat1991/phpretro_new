...
require_once('./templates/login_header.php');
	...
	if(isset($_POST['actionForgot'])){
		...
		$sql = $serverdb->query("SELECT id,name,email FROM ".PREFIX."users WHERE name = '".$forgot_name."' AND email = '".$forgot_mail."' ");
		...
		$lang->addLocale("forgot.email");
		$forgot_mail = $input->FilterText($_POST['ownerEmailAddress']);
