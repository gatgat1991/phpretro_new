...
if(!isset($_POST['page']) && @$session['page']){ $_POST['page'] = $_SESSION['page']; }
...
case "submit":
	...
	if(isset($_POST['username'])){ $name = $input->FilterText($_POST['username']); }
	if(isset($_POST['password'])){ $password = $input->HoloHash($_POST['password']); }
	
	
	if(isset($_POST['_login_remember_me'])){ $remember_me = $input->FilterText($_POST['_login_remember_me']); }
...
<?php
	...
	$_SESSION['user'] = $user;
	if(isset($_POST['page']) == true){ header("Location: ".PATH."/security_check?page=".$input->HoloText($_POST['page'])); exit; }elseif(isset($_SESSION['page']) == true){ header("Location: ".PATH."/security_check?page=".$_SESSION['page']); unset($_SESSION['page']); exit; }else{ header("Location: ".PATH."/security_check"); exit; }
}
break;
case "password":
